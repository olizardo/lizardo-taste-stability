...
# Replace Table 2 and Table 3 with \input{tex/...}
table2_start = content.find("Table 2.")
if table2_start != -1:
    table3_end = content.find(r"\end{longtable}", table2_start)
    if table3_end != -1:
        new_tables = r"""
...
"""
        content = content[:table2_start] + new_tables + content[table3_end + len(r"\end{longtable}"):]

img2_str = r"\includegraphics[width=6.81597in,height=1.83194in]{media/image2.wmf}"
content = content.replace(img2_str, "")
...
if "tabularray" not in content:
    ...
    # We might also need float for tabularray tables
    content = content.replace(r"\usepackage{tabularray}", r"\usepackage{tabularray,float}")
    # Also siunitx which modelsummary uses for \num
    content = content.replace(r"\usepackage{tabularray,float}", r"\usepackage{tabularray,float,siunitx}")
