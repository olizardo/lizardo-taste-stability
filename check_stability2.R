...
cat("Defined as: Starting at <= 2 (Very Often / Sometimes) and NOT dropping to >= 3 (Seldom/Never)\n\n")

results <- data.frame(Activity = character(), Retention = numeric(), N_At_Risk = numeric())

for (c in cultlist) {
  df_sub <- df_transitions %>% filter(.data[[paste0(c, "_start")]] <= 2) %>% drop_na(paste0(c, "_end"))
