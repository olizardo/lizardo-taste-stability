...
is observed in 1990 and a zero is recorded otherwise.\footnote{The
  ...
  and a standard deviation of 1.3.} In the regression model, \emph{$\gamma$} is
...
above), then we should observe \emph{$\gamma$}\textgreater0 for each of the
...
characteristics and \emph{$\beta$} is a \emph{k} x 1 vector of coefficients
...
majority of respondents retain their tastes ($\approx$98\%). When taste loss is
...
(\emph{$\gamma$\textsubscript{k }}\textgreater0, \emph{p}\textless0.05 for
...
(\emph{$\gamma$} also statistically significantly different from zero for all
...
In all looking at the relatively high $\chi$\textsuperscript{2} value for
...
period} ($\gamma$\textgreater0, \emph{p}\textless0.05). While not definitive
...
associations (\emph{$\gamma$}\textgreater0, \emph{p}\textless0.05). As Feld
...
% Appendix table temporarily disabled due to formatting errors
