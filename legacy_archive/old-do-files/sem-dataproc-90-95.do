use "N:\Private\taste-change\pc1990-95.dta", clear

*/creating cultural taste scales
forval i=4/5 {
	for var  music`i' movie`i' sports`i' paper`i' books`i' spevent`i' play`i' videos`i'  hobby`i' mags`i' tv`i'  friends`i':recode X 0=. 8/9=.
	}
for var art5 ballet5:recode X 0=. 8/9=.
...
tempvar rho rhose varname
qui gen `rho'=.
qui gen `rhose'=.
qui gen `varname'=""
local i=1
foreach name in music movie sports paper books spevent  play videos  hobby mags {	
	qui polychoric `name'4 	`name'5
	qui replace `rho'=r(rho) in `i'
	qui replace `rhose'=r(se_rho) in `i'
	qui replace `varname'="`name'" in `i'
	local i=`i'+1
	}
list `varname' `rho' `rhose' in 1/10,sep(0)
	

forval i=4/5 {
	egen numcultact`i'=rsum2(music`i'bin movie`i'bin sports`i'bin paper`i'bin books`i'bin spevent`i'bin  play`i'bin videos`i'bin  hobby`i'bin mags`i'bin),allmiss
	}
egen numcultact5alt=rsum2(music5 movie5 sports5 paper5 books5 spevent5  play5 videos5  hobby5 mags5 art5 ballet5),allmiss
...
global semvars    numcultact4 numcultact5 numcultact5alt friends5 friends4 numsocmems4 numsocmems5 music4 movie4 sports4 paper4 books4 spevent4  play4 videos4  hobby4 psport4 mags4 tv4  music5 movie5 sports5 paper5 books5 spevent5  play5 videos5  hobby5 psport5 mags5 tv5 female nonwhite age4 educ4 income4 married4 childre4 working4 bigcity4
