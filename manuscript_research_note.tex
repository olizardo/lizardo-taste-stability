...
For the frequency of interaction with friends, the survey asked respondents to indicate their level of interaction on an ordinal scale, with response categories originally coded such that 1 equaled ``Many'' (or high frequency) and 3 equaled ``Few'' (or low frequency) in earlier waves. This variable was programmatically harmonized and inverted prior to estimation so that higher integer values correctly reflect greater social connectivity. While originally ordinal, it is treated here as a count-like indicator of informal sociability in the Mundlak Poisson models for consistency with the formal membership count.
